...
Windows 11 supports only one interactive Remote Desktop session at a time. Concurrent independent RDP sessions require a Windows Server template configured as an RD Session Host and properly licensed with RDS CALs; replace the Windows 11 template reference when that environment is available. Do not patch `termsrv.dll` or use RDP Wrapper on competition guests.
